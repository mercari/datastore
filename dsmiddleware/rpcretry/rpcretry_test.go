...
func TestRPCRetry_waitDuration(t *testing.T) {
	specs := []struct {
		rh           *retryHandler
		retries      []int
		...
		{ // minBackoffDuration
			&retryHandler{
				minBackoffDuration: time.Millisecond,
			},
			[]int{
				1, 2, 3,
			},
		...
		{ // maxBackoffDuration
			&retryHandler{
				minBackoffDuration: time.Millisecond,
				maxBackoffDuration: 3 * time.Millisecond,
			},
			[]int{
				1, 2, 3,
			},
		...
		{ // maxDoublings
			...
			[]time.Duration{
				1 * time.Millisecond, 2 * time.Millisecond, 2 * time.Millisecond,
			},
		},
		{ // all zero
			&retryHandler{},
	...
	for specIdx, spec := range specs {
		for idx, retry := range spec.retries {
			expected := spec.expectedList[idx]

			ret := spec.rh.waitDuration(retry)
			if ret != expected {
				t.Errorf("#%d-%d expected: %d, actual: %d", specIdx+1, idx+1, expected, ret)
			}
		}
	}
}

func TestRPCRetry_Basic(t *testing.T) {
...
func TestRPCRetry_AllocateIDs(t *testing.T) {
	ctx, client, cleanUp := testutils.SetupCloudDatastore(t)
	defer cleanUp()
...
func TestRPCRetry_GetAll(t *testing.T) {
	ctx, client, cleanUp := testutils.SetupCloudDatastore(t)
	defer cleanUp()
...
func TestRPCRetry_Count(t *testing.T) {
	ctx, client, cleanUp := testutils.SetupCloudDatastore(t)
	defer cleanUp()
