...
https://godoc.org/go.mercari.io/datastore/v2/clouddatastore か https://godoc.org/go.mercari.io/datastore/v2/aedatastore を見てください。
...
詳細は後述します。
困ったら https://godoc.org/go.mercari.io/datastore/v2/clouddatastore を見ると解決の糸口があるかもしれません。
...
既に用意されているミドルウェアが知りたい場合は https://godoc.org/go.mercari.io/datastore/v2/dsmiddleware を参照してください。
...
これを実現したのが Batch() です。
https://godoc.org/go.mercari.io/datastore/v2/#pkg-examples に例があります。


goonを置き換えるboom

私はgoonが好きです。
ですので、本ライブラリと組み合わせて使える https://godoc.org/go.mercari.io/datastore/v2/boom を作りました。
...
AE Datastoreからの移行
	...
	go.mercari.io/datastore/v2 と go.mercari.io/datastore/v2/aedatastore をimportする。
	...
	google.golang.org/appengine/v2/datastore をimportしている箇所がないかチェックし、あれば go.mercari.io/datastore/v2 に置き換える。

Cloud Datastoreからの移行

	go.mercari.io/datastore/v2 と go.mercari.io/datastore/v2/clouddatastore をimportする。
	datastoreパッケージの関数を使っているものをFromContextとClientのメソッド呼び出しに書き換える。
	*datastore.Commit を datastore.Commit に置き換える。
	cloud.google.com/go/datastore をimportしている箇所がないかチェックし、あれば go.mercari.io/datastore/v2 に置き換える。
